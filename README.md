...
2. **Configure environment**
   ...
   - **LLM text generation (LangChain + LM Studio default):**
     - Start LM Studio's local server and load `openai/gpt-oss-20b`:
       ```bash
       lms server start
       lms load openai/gpt-oss-20b
       ```
     - Provider: `MANUSCRIPTAI_LLM_PROVIDER=langchain` (default). `lmstudio` is accepted as an alias.
...
- `./node_modules/.bin/vite-node --config vitest.config.ts scripts/verify-lmstudio-langchain.ts` – live-check the LangChain + LM Studio text-generation path against the local server
...
- General writing projects live at `/writing`, with a brief-first flow that feeds goals/outline context into LangChain-backed drafting and edits.
