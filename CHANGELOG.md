## 2025-12-13 02:12 UTC
- (Commit: codex/add-prosemirror-editor) Added a ProseMirror-powered editor surface with AI/user diff overlays, inline comment threads, and selection-aware controls for track-changes workflows.
- Introduced a ProseMirror preview page gated behind Clerk sign-in so the new editor, comment drawer, and AI batch-fix actions can be exercised in isolation.
- Styled ProseMirror decorations for user/LLM diffs and comment highlights to keep the new workspace visually aligned with the rest of the app while remaining readable.
